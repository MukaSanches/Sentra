...
public sealed class Employee : EntityBase
{
    ...
    public Employee(Guid condominiumId, Guid roleId, string fullName, string username, string passwordHash)
    {
        if (condominiumId == Guid.Empty) throw new ArgumentException("Condomínio inválido.", nameof(condominiumId));
        if (roleId == Guid.Empty) throw new ArgumentException("Perfil inválido.", nameof(roleId));
