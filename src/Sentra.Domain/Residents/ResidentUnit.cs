...
public sealed class ResidentUnit : EntityBase
{
    ...
    public ResidentUnit(Guid residentId, Guid unitId, ResidentUnitRole role, bool isPrimary = true)
    {
        if (residentId == Guid.Empty) throw new ArgumentException("Morador inválido.", nameof(residentId));
        if (unitId == Guid.Empty) throw new ArgumentException("Unidade inválida.", nameof(unitId));
