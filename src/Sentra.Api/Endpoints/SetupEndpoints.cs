...
{
    public static IEndpointRouteBuilder MapSentraSetupEndpoints(
        this IEndpointRouteBuilder endpoints)
    {
        endpoints.MapGet(
            "/api/v1/setup/status",
            async (SentraDbContext db, CancellationToken cancellationToken) =>
            {
                var bootstrapped = await db.Employees.AnyAsync(cancellationToken);
                return Results.Ok(new SetupStatusResponse(bootstrapped));
            })
            .AllowAnonymous();

        endpoints.MapPost("/api/v1/setup/bootstrap", BootstrapAsync)
            .AllowAnonymous();
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await db.Condominiums.AnyAsync(cancellationToken) ||
            await db.Employees.AnyAsync(cancellationToken))
        {
            return Results.Conflict(
                new { error = "O bootstrap inicial já foi executado." });
        }
