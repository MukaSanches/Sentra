...
| WhatsApp adapter oficial | IMPLEMENTADO | Sim | Graph client: texto/templates/interativos/Flows/mídia/read + parser/HMAC | credenciais Meta reais não fornecidas |
...
| Central Windows de conversas | EM VALIDAÇÃO | CI M2 | texto/template/Flow/interativos/mídia/SignalR | teste final do head em andamento |
| Mídia WhatsApp | IMPLEMENTADO OPERACIONALMENTE | Sim | upload/download/proxy autenticado + Media ID | retenção permanente depende de Object Storage |
